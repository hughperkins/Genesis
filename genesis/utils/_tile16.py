...
"""
Register-resident 16x16 tile with an optimised Cholesky factorization.

Local copy of `quadrants.lang.simt._tile16` differing from upstream only in the body
...
constraint Hessian. Exports `Tile16x16Cholesky`, a proxy with the same surface as
`quadrants.simt.Tile16x16` (`Tile16x16Cholesky.eye(dtype=...)`, `.zeros(dtype=...)`).
The distinct class name keeps it cleanly separated from quadrants' stock
`Tile16x16` — they are not interchangeable inside a single kernel because each is
...
# body) this collapses to a single field-reference AST node, vs. the 16-way `if k == 0: val = self.r0; if k == 1: ...`
...
def _make_tile16x16(dtype=None) -> "type[_Tile16x16Proto]":
    """Build (and memoize) a Tile16x16 dataclass with the optimised cholesky_."""
    if dtype is None:
        dtype = qd.f32
...
def _make_tile16x16_class(dtype):
    class _Tile16x16Cholesky:
        ...
        def _load(self, arr: qd.template(), row_start, row_stop, col_start, col_stop):
            ...
            if row < row_stop:
                ...
                # Avoids the 16x duplication that calling _set_col(j) through the @qd.func boundary would force.
        ...
        def cholesky_(self, eps):
            ...
            # getattr wrapper) rather than a 16-deep register-indexing cascade. Writes use an inline `if k == N:
        ...
        def solve_triangular_(self, B: Any, lower: bool = True) -> None:
            ...
            """
            if not lower:
                raise TypeError("Tile16x16Cholesky.solve_triangular_: only lower=True is supported")
            B._trsm(self)
        ...
        def _augassign(self, other: Any, op: str) -> None:
            ...
            if isinstance(other, _OuterProduct):
                if op == "Sub":
                    ...
                    raise TypeError(f"Tile16x16Cholesky: unsupported augmented assignment op '{op}' with outer product")
            else:
                raise TypeError(f"Tile16x16Cholesky: unsupported augmented assignment with {type(other)}")
    ...
    result = qd.dataclass(_Tile16x16Cholesky)
...
class _Tile16x16CholeskyProxy:
    """Proxy for dtype-at-point-of-use tile creation.

    Use as ``Tile16x16Cholesky.zeros(dtype=qd.f32)`` inside a kernel. The dtype is resolved at kernel compilation
    ...
    def _resolve(dtype):
        ...
        if arch in (qd.cpu, qd.x64, getattr(qd, "arm64", None)):
            raise QuadrantsSyntaxError(
                f"Tile16x16Cholesky requires a GPU backend (cuda, metal, vulkan, amdgpu). Current arch is {arch}."
            )
        if dtype is None:
...
# `qd.simt.Tile16x16` gets the same exemption only because of its module
# name; this restores parity.
_Tile16x16CholeskyProxy.zeros.__module__ = "quadrants.gen.tile16_cholesky"
_Tile16x16CholeskyProxy.eye.__module__ = "quadrants.gen.tile16_cholesky"


Tile16x16Cholesky = _Tile16x16CholeskyProxy()
