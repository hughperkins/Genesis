...
def get_constraint_state(constraint_solver, solver):
    ...
    return StructConstraintState(
        ...
        active=qd.tensor(gs.qd_bool, shape=(len_constraints_, _B), backend=_TENSOR_BACKEND, layout=con_layout),
        prev_active=V(dtype=gs.qd_bool, shape=(len_constraints_, _B)),
        diag=qd.tensor(gs.qd_float, shape=(len_constraints_, _B), backend=_TENSOR_BACKEND, layout=con_layout),
        aref=V(dtype=gs.qd_float, shape=(len_constraints_, _B)),
        Jaref=qd.tensor(gs.qd_float, shape=(len_constraints_, _B), backend=_TENSOR_BACKEND, layout=con_layout),
        efc_frictionloss=qd.tensor(
            gs.qd_float, shape=(len_constraints_, _B), backend=_TENSOR_BACKEND, layout=con_layout
        ),
        efc_force=V(dtype=gs.qd_float, shape=(len_constraints_, _B)),
        efc_D=qd.tensor(gs.qd_float, shape=(len_constraints_, _B), backend=_TENSOR_BACKEND, layout=con_layout),
        jv=qd.tensor(gs.qd_float, shape=(len_constraints_, _B), backend=_TENSOR_BACKEND, layout=con_layout),
