...
def func_cholesky_and_solve_fused_tiled(
    ...
    (``use_full_hessian[i_b] == 0`` and ``incr_n_changed[i_b] == 0``), the previous iter's L is loaded from the fp16
    ``nt_L_cache`` into shared memory and the factor loop is skipped entirely. Otherwise the factor loop runs as
    before and writes each completed L tile to both shared memory (for the substitution that follows) and
    ``nt_L_cache`` (for the next iter's skip path).

    The fp16 cache halves the bandwidth and shrinks the buffer enough to stay L2-resident across the solver loop's
    inter-cholesky kernels (see ``hessian_incremental_full.md`` Round 4 for the L2-eviction analysis that motivated
    the fp16 choice; the prior fp32 ``nt_L`` attempt cost -2.5% from cold-cache writes).

    The dispatcher in ``_func_build_changed_and_decide_hessian_mode`` already sets ``use_full_hessian == 1`` on
    iter <= 1, so the skip path is unreachable before ``nt_L_cache`` has been populated by iter-1's factor.
    """
    EPS = rigid_global_info.EPS[None]
    ...
    for i in range(_B * 16):
        ...
        if skip_factor:
            # Reuse previous iter's L: load tiles from nt_L_cache directly into L_sh, skip factor loop entirely.
            # Each tile is f16 in gmem, cast back to gs.qd_float in registers before being written to L_sh.
            for kb in range(N_BLOCKS):
                k0 = kb * 16
                k1 = qd.min(k0 + 16, n_dofs)

                L_kk = Tile16x16Cholesky.eye(dtype=gs.qd_float)
                L_kk._load3d_cast(constraint_state.nt_L_cache, qd.f16, i_b, k0, k1, k0, k1)
                ...
                for ib in range(kb + 1, N_BLOCKS):
                    ...
                    i1 = qd.min(i0 + 16, n_dofs)
                    L_ik = Tile16x16Cholesky.zeros(dtype=gs.qd_float)
                    L_ik._load3d_cast(constraint_state.nt_L_cache, qd.f16, i_b, i0, i1, k0, k1)
            ...
            for kb in range(N_BLOCKS):
                ...
                for ib in range(kb + 1, N_BLOCKS):
                    ...
                    # Write L[i,k] to shared memory (for in-kernel substitution) and to nt_L_cache as f16
                    # (for next-iter skip path; halves gmem write bandwidth vs an f32 cache).
                    L_ik._store(L_sh, i0, i1, k0, k1)
                    L_ik._store3d_cast(constraint_state.nt_L_cache, qd.f16, i_b, i0, i1, k0, k1)

                # Write L[k,k] to shared memory and to nt_L_cache as f16 (same rationale as L[i,k] above).
                L_kk._store(L_sh, k0, k1, k0, k1)
                L_kk._store3d_cast(constraint_state.nt_L_cache, qd.f16, i_b, k0, k1, k0, k1)
