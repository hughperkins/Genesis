...
def add_collision_constraints(
    ...
    max_contacts = collider_state.contact_data.link_a.shape[0]

    # Parallelized over (i_b, i_col) -- exposes B * n_contacts threads instead of just B.
    # Constraint indices are deterministic: base + i_col * 4 + i_dir, eliminating atomic_add.
    qd.loop_config(serialize=static_rigid_sim_config.para_level < gs.PARA_LEVEL.ALL)
    for i_b, i_col in qd.ndrange(_B, max_contacts):
        if i_col < collider_state.n_contacts[i_b]:
            # n_constraints[i_b] is stable here: set by prior add_frictionloss_constraints,
            # and this loop does not modify it.
    ...
    # Update n_constraints after all threads complete (implicit barrier between parallel for loops)
    qd.loop_config(serialize=static_rigid_sim_config.para_level < gs.PARA_LEVEL.ALL)
    for i_b in range(_B):
