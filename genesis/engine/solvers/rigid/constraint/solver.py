...
@func_solve_body.register(
    is_compatible=lambda *args, **kwargs: (
        _os_e5.environ.get("GS_E5_MEGAKERNEL", "0") == "1"
        and _os_e5.environ.get("GS_E5_BLOCK_DIM", "32") == "32"
        and not (cfg := _get_static_config(*args, **kwargs)).requires_grad
        and cfg.solver_type == gs.constraint_solver.Newton
        and not cfg.sparse_solve
...
@func_solve_body.register(
    is_compatible=lambda *args, **kwargs: (
        _os_e5.environ.get("GS_E5_MEGAKERNEL", "0") == "1"
        and _os_e5.environ.get("GS_E5_BLOCK_DIM", "32") == "64"
        and not (cfg := _get_static_config(*args, **kwargs)).requires_grad
        and cfg.solver_type == gs.constraint_solver.Newton
        and not cfg.sparse_solve
        and cfg.cholesky_tile_size == 32
...
@qd.kernel(fastcache=True)
def func_solve_body_megakernel_t32_b64(
    entities_info: array_class.EntitiesInfo,
    dofs_info: array_class.DofsInfo,
    dofs_state: array_class.DofsState,
    constraint_state: array_class.ConstraintState,
    ...
    """block_dim=64 variant of the t32 megakernel.

    Hypothesis: hessian rebuild + linesearch p0 benefit from 2x more lanes; the Tile32x32 cholesky factor stays
    at 32 lanes (sub-warp 1 redundantly computes the same factor — wasted work but no deadlock, since Tile32x32
    uses subgroup-internal ops). update_constraint_coop and cholesky body sub keep stride 32 to avoid cross-warp
    reduction refactor; sub-warp 1 does redundant work in those.

    Net hypothesis: hessian-rebuild speedup at block_dim=64 outweighs the redundant-work cost in the other phases.
    Enabled via GS_E5_MEGAKERNEL=1 GS_E5_BLOCK_DIM=64.
    """
    EPS_val = rigid_global_info.EPS[None]
    MAX_DOFS = qd.static(static_rigid_sim_config.tiled_n_dofs)
    _B = constraint_state.grad.shape[1]
    ...
    qd.loop_config(name="solve_megakernel_t32_b64", block_dim=64)
    for i_flat in range(_B * 64):
        tid = i_flat % 64
        i_b = i_flat // 64
        if i_b >= _B:
            continue
        ...
        # 64-lane shmem reductions for linesearch_p0_coop and eval (both templated, use shmem tree reduction).
        sh_ls0 = qd.simt.block.SharedArray((64,), gs.qd_float)
        sh_ls1 = qd.simt.block.SharedArray((64,), gs.qd_float)
        sh_ls2 = qd.simt.block.SharedArray((64,), gs.qd_float)
        sh_ls3 = qd.simt.block.SharedArray((64,), gs.qd_float)
        sh_ls4 = qd.simt.block.SharedArray((64,), gs.qd_float)
        sh_ls5 = qd.simt.block.SharedArray((64,), gs.qd_float)
        # Cross-warp scratch (subgroup ops cap at 32 lanes; we combine two 32-lane sums via shmem).
        sh_gn = qd.simt.block.SharedArray((2,), gs.qd_float)
                ...
                if local_iter >= rigid_global_info.iterations[None]:
                    ...
                    if constraint_state.improved[i_b]:
                        ...
                        if alpha_newton > 0.0:
                            ...
                            p1_alpha, p1_cost, p1_deriv_0, p1_deriv_1 = _func_linesearch_eval_at_alpha_coop16(
                                i_b, tid, alpha_newton, sh_ls0, sh_ls1, sh_ls2,
                                constraint_state, rigid_global_info, qd.static(64),
                            )
                            if p0_cost < p1_cost:
                                p1_alpha, p1_cost, p1_deriv_0, p1_deriv_1 = _func_linesearch_eval_at_alpha_coop16(
                                    i_b, tid, gs.qd_float(0.0), sh_ls0, sh_ls1, sh_ls2,
                            ...
                            if qd.abs(p1_deriv_0) > gtol:
                                # Refine bracketing + polish uses 32-lane subgroup ops. Sub-warp 1 does
                                # redundant work but reaches all syncs (no deadlock).
                                res_alpha, ls_result = func_linesearch_refine(
                                    i_b,
                                    tid,
                                    p1_alpha,
                    ...
                    if constraint_state.improved[i_b]:
                        ...
                        # update_constraint with _T=32, LOG2_T=5: sub-warp 1 redundantly processes constraints
                        # that lanes 0-31 also process (benign races on active/efc_force writes; identical values).
                        func_update_constraint_batch_coop(
                            i_b,
                            tid,
                            constraint_state.qacc,
                    ...
                    if constraint_state.improved[i_b]:
                        ...
                        # Cholesky body: stride 32 internally (Tile32x32 needs 32 lanes per subgroup); sub-warp 1
                        # redundantly computes the same factor.
                        func_cholesky_and_solve_body_skip_t32(
                            i_b,
                            tid,
                            L_sh,
                        ...
                        # Grad-norm: two-stage reduction (32-lane subgroup, then cross-warp via shmem).
                        gn_partial = gs.qd_float(0.0)
                        i_d_gn = tid
                        while i_d_gn < n_dofs_static:
                            g = constraint_state.grad[i_d_gn, i_b]
                            gn_partial = gn_partial + g * g
                            i_d_gn = i_d_gn + 64
                        gn_sub = qd.simt.subgroup.reduce_all_add_tiled(gn_partial, 5)
                        if tid == 0:
                            sh_gn[0] = gn_sub
                        if tid == 32:
                            sh_gn[1] = gn_sub
                        qd.simt.block.sync()
                        gn_total = sh_gn[0] + sh_gn[1]
