...
def func_cholesky_factor_direct_tiled(
    ...
    with qd.simt.Tile16x16 primitives (potrf, trsm, syr_sub, ger_sub), all operating entirely in registers via subgroup shuffles.
    ...
    _B = constraint_state.grad.shape[1]
    n_dofs = constraint_state.nt_H.shape[1]
    N_BLOCKS = (n_dofs + qd.simt.Tile16x16.SIZE - 1) // qd.simt.Tile16x16.SIZE

    qd.loop_config(name="cholesky_factor_direct_tiled", block_dim=qd.simt.Tile16x16.SIZE)
    for i in range(_B * qd.simt.Tile16x16.SIZE):
        tid = i % qd.simt.Tile16x16.SIZE
        i_b = i // qd.simt.Tile16x16.SIZE
        if i_b >= _B:
            continue
        if constraint_state.n_constraints[i_b] == 0 or not constraint_state.improved[i_b]:
            continue
        ...
        for kb in range(N_BLOCKS):
            ...
            for jb in range(kb):
                j0 = jb * qd.simt.Tile16x16.SIZE
                for t in range(qd.simt.Tile16x16.SIZE):
...
def func_cholesky_and_solve_fused_tiled(
    ...
    qd.loop_config(name="cholesky_and_solve_fused_tiled", block_dim=qd.simt.Tile16x16.SIZE)
    for i in range(_B * qd.simt.Tile16x16.SIZE):
        tid = i % qd.simt.Tile16x16.SIZE
        i_b = i // qd.simt.Tile16x16.SIZE
        if i_b >= _B:
