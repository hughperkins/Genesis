    ...
    def _build_static_config(self):
        ...
        if self.is_active:
            ...
            if gs.backend != gs.cpu:
                ...
                # Cooperative warp-per-env noslip (opt-in via GS_NOSLIP_COOP=1). Only when the cooperative constraint
                # layout is active and the whole-env sweep runs (per-island off), which is what kernel_noslip_coop
                # assumes. Non-bit-identical, so it stays off by default.
                rigid_config["enable_coop_noslip"] = (
                    enable_cooperative_constraint_kernels
