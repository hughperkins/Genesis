...
class Collider:
    ...
    def _init_collision_fields(self) -> None:
        ...
        if self._use_split_narrowphase:
            ...
            user_gjk_threads = self._solver._options.narrowphase_gjk_threads
            user_mpr_threads = self._solver._options.narrowphase_mpr_threads
            if gjk_only:
                self._multicontact_n_gjk_threads = _round_up_64(user_gjk_threads or gpu_cuda_cores)
                self._multicontact_n_total_threads = self._multicontact_n_gjk_threads
            else:
                self._multicontact_n_gjk_threads = _round_up_64(user_gjk_threads or (gpu_cuda_cores // 4))
                n_mpr = _round_up_64(user_mpr_threads or (gpu_cuda_cores * 7 // 4))
                self._multicontact_n_total_threads = self._multicontact_n_gjk_threads + n_mpr
