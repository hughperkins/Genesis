...
@qd.kernel(fastcache=gs.use_fastcache)
def func_broad_phase_sort_only(
    links_state: array_class.LinksState,
    links_info: array_class.LinksInfo,
    geoms_state: array_class.GeomsState,
    geoms_info: array_class.GeomsInfo,
    ...
    """
    Sort AABB endpoints along X-axis (first part of SAP).
    NOTE: Does NOT call func_collision_clear - that should be done before calling this.
    """
...
@qd.kernel(fastcache=gs.use_fastcache)
def func_broad_phase_sweep_candidates(
    links_state: array_class.LinksState,
    links_info: array_class.LinksInfo,
    geoms_state: array_class.GeomsState,
    geoms_info: array_class.GeomsInfo,
    ...
    """
    Sweep sorted endpoints to generate candidates (no validation).
    """
    n_geoms, _B = collider_state.active_buffer.shape
    n_links = links_info.geom_start.shape[0]
...
def func_broad_phase_validate_candidates(
    ...
    qd.loop_config(serialize=static_rigid_sim_config.para_level < gs.PARA_LEVEL.ALL)
    for i_b, i_cand in qd.ndrange(_B, max_candidates):
        ...
        idx = qd.atomic_add(collider_state.n_broad_pairs[i_b], 1)
...
@qd.kernel(fastcache=gs.use_fastcache)
def func_broad_phase_sap_sort_only(
    links_state: array_class.LinksState,
    links_info: array_class.LinksInfo,
    geoms_state: array_class.GeomsState,
    geoms_info: array_class.GeomsInfo,
    ...
    """
    SAP sort phase only (for profiling).
    Identical to the sort portion of func_broad_phase, including func_collision_clear.
    """
    n_geoms, _B = collider_state.active_buffer.shape
    n_links = links_info.geom_start.shape[0]
...
@qd.kernel(fastcache=gs.use_fastcache)
def func_broad_phase_sap_sweep_only(
    links_state: array_class.LinksState,
    links_info: array_class.LinksInfo,
    geoms_state: array_class.GeomsState,
    geoms_info: array_class.GeomsInfo,
    ...
    """
    SAP sweep phase only (for profiling).
    Identical to the sweep portion of func_broad_phase.
    """
    n_geoms, _B = collider_state.active_buffer.shape
    n_links = links_info.geom_start.shape[0]
