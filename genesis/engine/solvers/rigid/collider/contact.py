...
def func_clamp_and_sort_contacts(
    ...
    for i_b in range(_B):
        ...
        # deskai6 opt 8b: skip permuting force & pair_idx when not requires_grad. Same reasoning as
        # opt 8a in func_prune_contacts phase 3: force is zeroed at step start and re-written by the
        # solver, pair_idx is only read by the differentiable narrowphase. clamp_and_sort runs in
        # both grad and non-grad paths so we gate on requires_grad.
        for i in range(n_con):
...
def func_prune_contacts(
    ...
    tolerance of their mean and whose positions lie in a single plane perpendicular to that mean
    normal, keeps only the 2D convex hull vertices of the projected positions. Buckets that fail
    either coplanarity gate are left untouched: the support-polygon equivalence requires both shared
    normal direction and a single contact plane (no parallel layers).

    The single ``tol`` parameter controls both gates as a dimensionless slop fraction:
      - normals: |sin(angle from mean)| <= tol (i.e. angle <= arcsin(tol))
      - depth:   max |out-of-plane offset| / in-plane radius <= tol

    A reasonable starting value is 0.1 (around 5.7 degrees, 10 percent depth ratio).
    ...
    2. Per bucket >= 5 contacts: compute mean normal (folded to a common hemisphere), check angular and depth
       coplanarity, project to (u, v), Andrew's monotone chain. Mark survivors in contact_keep[].
        ...
        if n_con >= 5:
            ...
            while b_start < n_con:
                ...
                if b_size >= 5:
                    ...
                    # POST-REDUCE math runs on all 32 lanes (deterministic, cheap; redundant
                    # arithmetic is free vs broadcasting the reduce results).
                    ...
                    if coplanar:
                        ...
                        # COOP coplanarity check (stage 3). Each lane strides [b_start + tid,
                        # b_end) by _K, locally tracking coplanar / max_depth / max_in_plane_r2.
                        # The serial early-exit `break` is dropped — all lanes visit every
                        # assigned contact so the reductions are well-defined. Wasted work per
                        # warp is at most b_size/_K contacts.
                        coplanar_l = True
                        ...
                        while jj < b_end:
                            src_i = collider_state.contact_sort_idx[jj, i_b]
                            n_i = collider_state.contact_data.normal[src_i, i_b]
                            if qd.abs(mnx * n_i[0] + mny * n_i[1] + mnz * n_i[2]) < cos_normal_tol:
                                coplanar_l = False
                            p_i = collider_state.contact_data.pos[src_i, i_b]
                            dx = p_i[0] - cx
                            dy = p_i[1] - cy
                        ...
                        coplanar = qd.simt.subgroup.all_true_tiled(coplanar_l, 5)
                        ...
                        if coplanar and max_depth > tol * qd.sqrt(max_in_plane_r2):
                            coplanar = False

                    if coplanar:
                        # Basis on all lanes (deterministic from mnx/mny/mnz which the reduce
                        ...
                        # SYNC between coop writes (sort_key, proj_v, lex_idx, contact_keep[orig])
                        # and the lane-0 lex sort + hull build that reads them.
                        qd.simt.subgroup.sync()

                    if tid == 0 and coplanar:
                        # SERIAL on lane 0: lex sort + Andrew monotone-chain hull build + mark
                        # survivors. These walk a stack and have data-dependent inner loops that
                        # don't decompose across warp lanes.
