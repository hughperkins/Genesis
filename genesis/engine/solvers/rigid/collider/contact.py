...
def func_prune_contacts(
    ...
    A reasonable starting value is 0.1 (10 percent depth ratio).
    ...
    for i_b in range(_B):
        ...
        # Default: keep everything. Buckets that pass the gates flip their entries to drop and then
        # mark only hull-vertex contacts as keep again.
        for i in range(n_con):
        ...
        while b_start < n_con:
            ...
            if b_size >= 5:
                ...
                if coplanar:
                    ...
                    # Depth coplanarity: positions must lie in a single plane perpendicular to the mean normal.
                    # No per-contact normal check: a contact whose normal is diagonal (e.g. an edge-vs-edge
                    # contact at a corner of the contact patch) still participates in the 2D hull because its
                    # position is a vertex of the patch; dropping a collinear-edge contact in the same bucket is
                    # justified by the positional support polygon regardless of that contact's normal direction.
                ...
                if coplanar:
                    ...
                    # Sort bucket indices lexicographically by (u, v), with a tolerance on u so that contacts
                    ...
                    # triplet when the corner and the mid-edge have u values that differ by a few microns and the
                    # mid-edge happens to sort first.
                    ...
                    for k_step in range(b_size - 1):
                        ...
                        # stack[b_start] from the lower hull. Skipping that push, plus the k < b_size guard, bounds k
                        # to b_size and keeps the write index within max_contact_pairs even for buckets where the
                        # lower-hull pass already kept all b_size points (downward-convex layouts: every lex-sorted
                        # triple makes a left turn so nothing gets popped, then the upper-hull pass tries to push a
                        # duplicate of an already-kept lower-hull vertex).
