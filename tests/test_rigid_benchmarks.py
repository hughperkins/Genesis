...
def run_benchmark(step_fn, *, n_envs, meta):
    ...
    while True:
        ...
        if is_recording:
            num_steps += 1
            if diag_enabled and num_steps % diag_interval == 0:
                qd.sync()
                t_sync = time.time() - time_start
                diag_entries.append({"phase": "record", "step": num_steps, "wall_nosync": time_elapsed,
                                     "wall_sync": t_sync})
            ...
            warmup_steps += 1
            if diag_enabled and warmup_steps % diag_interval == 0:
                qd.sync()
                t_sync = time.time() - time_start
                diag_entries.append({"phase": "warmup", "step": warmup_steps, "wall_nosync": time_elapsed,
                                     "wall_sync": t_sync})
            if time_elapsed > meta.duration_warmup:
                qd.sync()
                time_start = time.time()
                is_recording = True
